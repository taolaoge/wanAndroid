...
class Fragment1 : Fragment() {
    ...
    private fun loadMoreData() {
        val prefs=activity?.getSharedPreferences("cookie",Context.MODE_PRIVATE)
        val cookie=prefs?.getString("cookie","")?:""
        HttpUtil.sendOkHttpGetRequest("https://www.wanandroid.com/project/list/1/json?cid=294", cookie,
            object : Callback {
    ...
    private fun recycleViewData(responseData: String?) {
        ...
        for (i in 0..size) {
            val envelopPic=articleResponse.data.datas!![i].envelopePic
            ...
            val desc=articleResponse.data.datas!![i].desc
